...
class Layer():
    ...
    def __line_between_two_neurons(self, neuron1, neuron2, linewidth, sign):
        ...
        line = pyplot.Line2D(line_x_data, line_y_data, linewidth=linewidth, color=lineColor, alpha=.4)
        line.set_zorder(1)
...
def visualize_model( module ):
    modelParams = module.get_params()[0]
    nLayers = 0
    finalLayerSize = 0
    modelShape = {}
    modelWeights = {}
    ...
    for k,v in iter(sorted(modelParams.items())):
        if 'weight' in k.lower():
            modelShape[nLayers] = v.shape[1]
            modelWeights[nLayers] = v.asnumpy()
            finalLayerSize = v.shape[0]
            nLayers += 1
            #print( v.shape)
            #print( v.asnumpy() )
    ...
    # first layer
    network.add_layer( modelShape[0], modelWeights[0])

    for iLayer in range( 1, nLayers ):    
        network.add_layer( modelShape[iLayer], modelWeights[iLayer])

    # last layer
    network.add_layer( finalLayerSize )
